...
class MainMenuFragment : Fragment(R.layout.fragment_launcher) {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        try {
            val testFile = requireContext().resources.assets.open("test.txt")
            val testOut = FileOutputStream("testOut.txt")
            val bos = BufferedOutputStream(testOut)

            val buffer = ByteArray(65535)
            var current = testFile.read(buffer)
            var overall = 0
            while (current != -1) {
                overall += current
                bos.write(buffer, 0, current)
                current = testFile.read(buffer)
            }
            bos.flush()
            bos.close()
            testOut.close()
